...
### Changed
- Имя HTML-отчёта: `net_diag_<время>_<локальный-IP>_<внешний-IP>.html` (если IP нет — `no-lan` / `no-wan`).

## [1.5.5] — 2026-08-07

### Fixed
- Windows: ping DNS/шлюза больше не показывает ложный `loss=100%` на русской ОС — ICMP через `IcmpSendEcho` вместо парсинга локализованного `ping.exe` (OEM ≠ UTF-8).
...
### Changed
- GUI: отдельные логи на вкладках «Диагностика», «Пробы» и «URL»; «Очистить лог» чистит только текущую.

## [1.5.2] — 2026-08-05

### Fixed
- GUI: интервал и раунды на вкладках «Пробы» / «URL» — обычные поля ввода без кнопок ±, каждое в своей строке (больше не наползают).

## [1.5.1] — 2026-08-05

### Fixed
- GUI: кнопка «Открыть отчёт» — абсолютный путь к HTML, проверка что файл есть, понятная ошибка в логе; fallback записи в `~/Documents/ConnectCheck` если каталог пакета read-only (DMG).

## [1.5.0] — 2026-08-05

### Fixed
- GUI: окно логов больше не сжимается по мере появления этапов — чеклист в фиксированной прокручиваемой панели, пол высоты лога 220px.

### Changed
- GUI: все этапы диагностики показываются сразу (○ pending / → running / ✓ done / ⏭ skipped); план обновляется при смене чекбоксов.
- Движок: `cc_engine_stages()` — каталог этапов; ранние блоки «Сеть и Wi‑Fi», «Captive / OS», «NTP» идут через `stage_begin` (как остальные).
...
### Changed
- macOS: обычный двойной клик по `ConnectCheck-mac.app` — встроенный лаунчер сам снимает quarantine; `.command` больше не нужен и не кладётся в архив.
